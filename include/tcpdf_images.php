...
class ImageHelper {
    ...
    public static function getImageFileType($imgfile, $iminfo=array()) {
        ...
        if (empty($type)) {
            ...
            if (isset($fileinfo['extension']) and (!StaticHelpers::empty_string($fileinfo['extension']))) {
	...
	public static function _parsejpeg($file) {
		...
		while (($pos = strpos($data, "ICC_PROFILE\0", $offset)) !== false) {
			...
			$length = (StaticHelpers::_getUSHORT($data, ($pos - 2)) - 16);
	...
	public static function _parsepng($file) {
		...
		$w = StaticHelpers::_freadint($f);
		$h = StaticHelpers::_freadint($f);
		$bpc = ord(fread($f, 1));
		$ct = ord(fread($f, 1));
		if ($ct == 0) {
			$colspace = 'DeviceGray';
		...
		$data = '';
		$icc = false;
		$n = StaticHelpers::_freadint($f);
		do {
			$type = fread($f, 4);
			if ($type == 'PLTE') {
				// read palette
				$pal = StaticHelpers::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'tRNS') {
				// read transparency info
				$t = StaticHelpers::rfread($f, $n);
				if ($ct == 0) { // DeviceGray
					$trns = array(ord($t[1]));
				...
				$icc = StaticHelpers::rfread($f, ($n - $len - 2));
				// decompress profile
