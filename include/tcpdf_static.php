...
class StaticHelpers
{
    ...
    {
        return tempnam(K_PATH_CACHE, '__tcpdf_'.$file_id.'_'.$type.'_'.md5(StaticHelpers::getRandomSeed()).'_');
    }
    ...
    * @since 5.9.121 (2011-09-28)
     ...
     * @public static
     */
    public static function AESnopad($key, $text)
    {
        if (extension_loaded('openssl')) {
     ...
     * @public static
     */
    public static function RC4($key, $text, &$last_enc_key, &$last_enc_key_c)
    {
        if (function_exists('mcrypt_encrypt') &&
            ($out = @mcrypt_encrypt(MCRYPT_ARCFOUR, $key, $text, MCRYPT_MODE_STREAM, ''))
        ) {
     ...
     * @param $prop array javascript field properties.
    ...
    public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl = false)
    {
        ...
        // buttonPosition: Controls how the text and the icon of the button are positioned with respect to each other
        ...
        // strokeColor: Specifies the stroke color for a field that is used to stroke the rectangle of the field
        ...
        // doNotScroll: If true, the text field does not scroll and the user, therefore, is limited
        //by the rectangular region designed for the field.
        if (isset($prop['doNotScroll']) and ($prop['doNotScroll'] == 'true')) {
            $ff += 1 << 23;
        }
        // comb: If set to true, the field background is drawn as series of boxes
        //(one for each character in the value of the field) and each character of the content is drawn
        //within those boxes. The number of boxes drawn is determined from the charLimit property.
        //It applies only to text fields.
        //The setter will also raise if any of the following field properties are also set
        //multiline, password, and fileSelect.
        //A side-effect of setting this property is that the doNotScroll property is also set.
        if (isset($prop['comb']) and ($prop['comb'] == 'true')) {
            $ff += 1 << 24;
        }
        // radiosInUnison: If false, even if a group of radio buttons have the same name and export value,
        ...
        // readonly: The read-only characteristic of a field.
        ...
        // submitName: If nonempty, used during form submission instead of name.
     ...
     * @param $file (string) TEX file containing hypenation patterns.
    ...
    public static function getHyphenPatternsFromTEX($file)
    {
        ...
        foreach ($patterns_array as $val) {
            if (!StaticHelpers::empty_string($val)) {
                $val = trim($val);
                $val = str_replace('\'', '\\\'', $val);
     ...
     * @param $limit int If specified, then only substrings up to limit are returned with the rest of the string
     * being placed in the last substring. A limit of -1, 0 or NULL means "no limit" and, as is standard across PHP,
